...
| 27 | `json-parse` | `fn(text: *mut RtValue) -> *mut RtValue` returning `Result` (`Ok(value)` / `Err("json-parse: invalid JSON: ...")`) | `stdlib/json.airl` | Before fix: `:sig [(s : String) -> Any]` returning raw Any; after fix: `:sig [(s : String) -> Result]` returning `(Ok value)` | ❌ Drift (fixed) | NOT auto-included; accessible via `DEPS: stdlib/json.airl` or explicit import. Callers matching on `(Ok v)` / `(Err e)` would fail with raw Any. Fixed in this PR: `json-parse` now wraps success in `(Ok value)`. Note: pure-AIRL parser cannot detect malformed JSON at parse-value level, so this implementation always returns `(Ok ...)` — it does not produce `(Err ...)` for invalid input. This is a known limitation noted in Follow-up. |
