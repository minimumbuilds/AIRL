...
| ⚠️ Intentional | 4 |
| Unreachable (neither auto-included nor imported) | 0 |

Unreachable count closed via base64 completeness PR 2026-04-23.

**Auto-included stdlib modules** (from `STDLIB_MODULES` in `pipeline.rs`):
`prelude.airl`, `math.airl`, `result.airl`, `string.airl`, `map.airl`, `set.airl`, `io.airl`, `path.airl`, `random.airl`, `json.airl`, `base64.airl`, `sqlite.airl`
...
| 27 | `json-parse` | `fn(text: *mut RtValue) -> *mut RtValue` returning `Result` (`Ok(value)` / `Err("json-parse: invalid JSON: ...")`) | `stdlib/json.airl` | Before fix: `:sig [(s : String) -> Any]` returning raw Any; after fix: `:sig [(s : String) -> Result]` returning `(Ok value)` on success, `(Err "json-parse: empty input")` on empty input | ✅ Parity | Auto-included via STDLIB_MODULES entry (added 2026-04-23 — see docs/superpowers/specs/2026-04-23-base64-completeness-design.md). Full error propagation implemented 2026-04-23: malformed non-empty input returns `(Err "json-parse: ...")` for unterminated strings/arrays/objects, invalid numbers, expected-value-got-something-else, and unexpected trailing content. Now matches the Rust builtin's error semantics. Exhaustive cases tested in `round3_builtin_json_parse_errors.airl`. |
| 28 | `json-stringify` | `fn(val: *mut RtValue) -> *mut RtValue` returning raw String | `stdlib/json.airl` | `:sig [(val : Any) -> String]` returning raw String | ✅ Parity | Auto-included via STDLIB_MODULES. Both return raw JSON string. |
...
| 34 | `base64-encode` | `fn(s: *mut RtValue) -> *mut RtValue` returning raw String | `stdlib/base64.airl` | `:sig [(s : String) -> String]` returning raw String | ✅ Parity | Auto-included via STDLIB_MODULES entry (added 2026-04-23 — see docs/superpowers/specs/2026-04-23-base64-completeness-design.md). Both encode string to Base64. |
| 35 | `base64-decode` | `fn(s: *mut RtValue) -> *mut RtValue` returning raw String (empty on error) | `stdlib/base64.airl` | `:sig [(s : String) -> String]` returning raw String | ✅ Parity | Auto-included via STDLIB_MODULES entry (added 2026-04-23 — see docs/superpowers/specs/2026-04-23-base64-completeness-design.md). Both return decoded string; both return empty string on invalid input (no error path). |
| 36 | `base64-encode-bytes` | `fn(data: *mut RtValue) -> *mut RtValue` returning raw Bytes | None (no AIRL defn) | N/A | ✅ Parity | Rust function `airl_base64_encode_bytes` (airl-rt/src/misc.rs) re-registered as builtin in bytecode_vm.rs and bytecode_aot.rs on 2026-04-23 after this audit flagged it as unreachable. AIRL stdlib has no Bytes-variant implementation; Rust is the sole provider. Parity: the Bytes-to-Bytes contract of the Rust function is fully preserved by re-registration. Airlos stub added to misc.rs so bytecode_aot.rs declare_import is unconditional. |
| 37 | `base64-decode-bytes` | `fn(data: *mut RtValue) -> *mut RtValue` returning raw Bytes | None (no AIRL defn) | N/A | ✅ Parity | Rust function `airl_base64_decode_bytes` (airl-rt/src/misc.rs) re-registered as builtin in bytecode_vm.rs and bytecode_aot.rs on 2026-04-23 after this audit flagged it as unreachable. AIRL stdlib has no Bytes-variant implementation; Rust is the sole provider. Parity: the Bytes-to-Bytes contract of the Rust function is fully preserved by re-registration. Airlos stub added to misc.rs so bytecode_aot.rs declare_import is unconditional. |
...
1. ~~**`base64-encode-bytes` / `base64-decode-bytes` missing AIRL replacements (rows 36-37).**~~ **Resolved 2026-04-23 in `fix/base64-completeness`.** The Rust builtins were re-registered in both dispatch tables. No AIRL Bytes-variant implementations are needed; the Rust functions are the correct providers for Bytes→Bytes base64 (writing pure-AIRL byte-level base64 would risk OOM on large payloads, as seen with SHA256).
...
5. ~~**`json.airl` and `base64.airl` auto-include consideration.**~~ **Resolved 2026-04-23.** Both are now auto-included via `STDLIB_MODULES` entries added in the consolidation refactor (`json.airl`) and the base64 completeness fix (`base64.airl`). The Rust builtins for `json-parse`, `json-stringify`, `base64-encode`, `base64-decode` are deregistered and their AIRL replacements in `stdlib/json.airl` and `stdlib/base64.airl` are now globally available without explicit import.
