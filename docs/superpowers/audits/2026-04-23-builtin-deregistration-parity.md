...
| 27 | `json-parse` | `fn(text: *mut RtValue) -> *mut RtValue` returning `Result` (`Ok(value)` / `Err("json-parse: invalid JSON: ...")`) | `stdlib/json.airl` | Before fix: `:sig [(s : String) -> Any]` returning raw Any; after fix: `:sig [(s : String) -> Result]` returning `(Ok value)` on success, `(Err "json-parse: empty input")` on empty input | ❌ Drift (fixed) | NOT auto-included; accessible via `DEPS: stdlib/json.airl` or explicit import. Callers matching on `(Ok v)` / `(Err e)` would fail with raw Any. Fixed in this PR: `json-parse` now wraps success in `(Ok value)` and returns `(Err "json-parse: empty input")` for empty input, matching the Rust builtin's semantics (and the `fix/stdlib-json-autoinclude` branch's independently-chosen behavior for merge compatibility). Note: the pure-AIRL parser cannot detect malformed non-empty JSON at parse-value level, so invalid non-empty input still returns `(Ok ...)` with a best-effort value. This is a known limitation noted in Follow-up. |
...
2. **`json-parse` error detection incomplete.** The pure-AIRL recursive-descent parser (`json-parse-value`) does not propagate parse errors; it always returns a best-effort value. The `(Err ...)` branch of the `Result` returned by `json-parse` is never actually produced by the AIRL implementation (unlike the Rust version which returned `Err` for truly invalid JSON). A future PR could add an explicit validation pass or add error propagation to `json-parse-value`. Until then, callers should treat `(Err ...)` from `json-parse` as "not reachable in practice" but write defensive code that handles it anyway.
