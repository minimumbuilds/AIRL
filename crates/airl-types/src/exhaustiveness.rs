...
pub fn check_exhaustiveness(
    ...
    match scrutinee_ty {
        ...
        Ty::Sum(variants) => check_sum_exhaustiveness(variants, arms, env),
...
fn check_sum_exhaustiveness(
    ...
    let missing: Vec<String> = variants
        .iter()
        .filter(|v| !covered.contains(env.interner.resolve(v.name)))
        .map(|v| env.interner.resolve(v.name).to_owned())
        .collect();
...
mod tests {
    ...
    /// Build a Result-like sum type, interning "Ok" and "Err" via the env.
    fn result_type(env: &mut TypeEnv) -> Ty {
        let ok_id = env.intern("Ok");
        let err_id = env.intern("Err");
        Ty::Sum(vec![
            TyVariant { name: ok_id, fields: vec![Ty::Prim(PrimTy::I32)] },
            TyVariant { name: err_id, fields: vec![Ty::Prim(PrimTy::Str)] },
        ])
    }
    ...
    fn result_only_ok_is_missing_err() {
        ...
        let err = check_exhaustiveness(&ty, &arms, &env).unwrap_err();
