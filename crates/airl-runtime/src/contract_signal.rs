//! Shared contract-error signaling for AOT/native-compiled code.
//!
//! The AOT path calls `airl_jit_contract_fail` from native code when a
//! contract assertion fails. The error is stored in a thread-local cell
//! and retrieved by the bytecode VM after the native call returns.
...
/// C-ABI function called by AOT/native-compiled code when a contract assertion fails.
...
/// Check if a contract error was signaled and extract it.
