...
2. **`length` vs `char-at` indexing mismatch on non-ASCII strings:** The `length` builtin returns **byte length** but `char-at` uses **character (Unicode codepoint) indexing**. For ASCII-only strings they agree, but any multi-byte character (em-dash `—`, accented letters, emoji, etc.) causes `length` to overcount. Code that loops `pos` from 0 to `(length s)` and calls `(char-at s pos)` will crash with an out-of-bounds error on non-ASCII input. **Affected code:** The bootstrap lexer (`bootstrap/lexer.airl`) uses this pattern in `skip-ws`, `read-digits`, `read-string`, `read-symbol`, and `next-token`. It works correctly on ASCII-only `.airl` files but will crash on source files containing multi-byte UTF-8 characters (including in comments). **Fix options:** (a) Change the Rust `builtin_length` for strings to return character count instead of byte count (`s.chars().count()`), or (b) add a `char-count` builtin and update the bootstrap lexer to use it, or (c) make `char-at` use byte indexing. Option (a) is the simplest and least disruptive — the byte-length behavior is undocumented and unlikely to be relied upon. The fix is in `crates/airl-runtime/src/builtins.rs`, function `builtin_length`, the `Value::Str` arm.
