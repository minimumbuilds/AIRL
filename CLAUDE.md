...
- **HTTP Request Builtin** — Generic `(http-request method url body headers)` supporting GET, POST, PUT, DELETE, PATCH, HEAD. Returns `Result[Str, Str]`. Uses `ureq` in Rust, `libcurl` with `CURLOPT_CUSTOMREQUEST` in C runtime.
